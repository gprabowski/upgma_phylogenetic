...
def get_alignment(matrix, a : str, b : str, x : int, y : int):
    ...
    while(x != 0 or y != 0):
        ...
        if(matrix[y][x][0][0] == Arrow.NW):
            ...
            y -= 1
    return fw1, fw2
def make_alignment(first : str, second : str, idx : int, idx2 : int, l_idx : int, gap_start : int, gap_continue : int, arr):
    ...
    v_matrix = [[0 for x in range(len(first) + 1)] for y in range(len(second) + 1)]
    ...
    arr[idx * l_idx + idx2] = v_matrix[len(second)][len(first)][1]
